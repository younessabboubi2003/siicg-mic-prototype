...
st.set_page_config(
    ...
    page_icon="logo_mic.png",
    layout="wide",
    initial_sidebar_state="expanded",
)

# ============================================================
# PREMIUM CONSULTING PALETTE
# ============================================================
NAVY = "#1E2A44"
NAVY_2 = "#243B63"
GOLD = "#C8A96B"
...
MIC_LOGO = "logo_mic.png"
DEFAULT_FILE = "kpi_mic_sample.csv"

# ============================================================
# CSS — EXECUTIVE / CONSULTING LOOK
# ============================================================
st.markdown(f"""
<style>
html, body, [class*="css"] {{
...
def performance_ratio(row):
    ...
    Exemple : délai réel 25 jours / cible 30 jours => performance > 100%.
    """
...
# TABS
# ============================================================
tab_exec, tab_archi = st.tabs(["Cockpit exécutif", "Architecture SIICG"])
...
with tab_exec:
    ...
    with col_type:
        type_counts = filtered["type"].value_counts().reset_index() if len(filtered) else pd.DataFrame({"type": [], "count": []})
        type_counts.columns = ["type", "nb"] if len(type_counts) else ["type", "nb"]
        if len(type_counts):
            fig_type = px.pie(
                type_counts,
                names="type",
...
with tab_archi:
    ...
    <div style="background:white;border:1px solid {LGRAY};border-radius:12px;padding:24px;box-shadow:0 2px 8px rgba(30,42,68,0.045);">
      <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:10px;align-items:stretch;text-align:center;">
        ...
        <div style="background:{NAVY};color:white;border-radius:10px;padding:18px;">
            <b>SIICG</b><br><span style="font-size:12px;color:#CBD5E1;">Cockpit · alertes · RPM · rapports</span>
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="background:white;border:1px solid {LGRAY};border-radius:12px;padding:20px;">
      <ol style="color:{NAVY};line-height:1.9;">
        ...
        <li><b>Analyser</b> les écarts en vue des Revues de Performance Mensuelles.</li>
