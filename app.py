...
def get_image_base64(path):
    """Convert local image to base64 for reliable rendering in Streamlit HTML."""
    try:
...
MIC_LOGO_SRC = get_image_base64(MIC_LOGO)
...
/* Force sidebar visibility and stable width */
section[data-testid="stSidebar"] {{
...
<div class="header-band">
  <div class="header-left">
    <img src="{MIC_LOGO_SRC}" class="header-logo" alt="Logo MIC"/>
    <div>
      <div class="header-title">SIICG — Plateforme de Pilotage de la Performance Publique</div>
...
<b>Démonstrateur fonctionnel SIICG.</b> Cette version de cadrage illustre la transformation du reporting de performance en un dispositif intégré de pilotage, d’alerte, d’analyse, d’arbitrage et de capitalisation documentaire au service des instances de gouvernance.
</div>
""", unsafe_allow_html=True)
...
def generate_report(df):
    ...
    lines += [
        ...
        "",
        "──────────────────────────────────────────────────────────────",
        "Prototype académique SIICG — PFE Version de cadrage × MIC",
    ]
...
with st.sidebar:
    ...
    <div class="sidebar-logo-wrap">
      <img src="{MIC_LOGO_SRC}" class="sidebar-logo" alt="Logo MIC"/>
    </div>
    <div class="sidebar-doc-title">SIICG — Pilotage Performance</div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown(
        '<div style="font-size:11px;color:#CBD5E1;text-align:center;">Usage interne — Données démonstratives<br>Version de cadrage fonctionnel · 2026</div>',
        unsafe_allow_html=True
    )
...
# TABS
# ============================================================
tab_home, tab_exec, tab_archi, tab_arbitrage, tab_heatmap, tab_rpm, tab_docs = st.tabs(["À propos du SIICG", "Cockpit exécutif", "Architecture SIICG", "Centre d’Arbitrage", "Performance Heatmap", "RPM Executive Mode", "Référentiel documentaire"])


with tab_home:
    st.markdown('<div class="section-title">SIICG — Système d’Information Intégré de Contrôle de Gestion</div>', unsafe_allow_html=True)

    st.markdown("""
    <div class="method-box">
    Le SIICG est conçu comme une plateforme intégrée de pilotage de la performance publique permettant de
    centraliser les indicateurs, qualifier les alertes, structurer les arbitrages et capitaliser le référentiel
    réglementaire et méthodologique du dispositif de contrôle de gestion.
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="exec-grid">
      <div class="exec-card gold">
        ...
        <div class="value">Piloter</div>
        <div class="sub">Transformer les données KPI en décisions</div>
      </div>
      <div class="exec-card green">
        <div class="label">Périmètre</div>
        <div class="value">MIC</div>
        <div class="sub">Programmes, directions, DOCG et RPM</div>
      </div>
      <div class="exec-card orange">
        <div class="label">Usage</div>
        <div class="value">Interne</div>
        <div class="sub">Version de cadrage fonctionnel</div>
      </div>
      <div class="exec-card gold">
        <div class="label">Données</div>
        <div class="value">Démo</div>
        <div class="sub">Jeu de données démonstratif</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown('<div class="section-title">Finalités du dispositif</div>', unsafe_allow_html=True)
        st.markdown("""
        <div style="background:white;border:1px solid #E5E7EB;border-radius:12px;padding:18px;line-height:1.8;">
        <b>1. Centraliser</b> les données de performance issues des programmes et directions.<br>
        <b>2. Analyser</b> automatiquement les écarts entre réalisations et cibles.<br>
        <b>3. Alerter</b> selon des seuils de criticité et des niveaux d’escalade.<br>
        <b>4. Arbitrer</b> les actions correctives via les Revues de Performance Mensuelles.<br>
        <b>5. Capitaliser</b> le cadre réglementaire, méthodologique et documentaire du contrôle de gestion.
        </div>
        """, unsafe_allow_html=True)

    with col2:
        st.markdown('<div class="section-title">Utilisateurs cibles</div>', unsafe_allow_html=True)
        st.markdown("""
        <div style="background:white;border:1px solid #E5E7EB;border-radius:12px;padding:18px;line-height:1.8;">
        <b>DOCG :</b> animation du dispositif, consolidation et analyse des KPI.<br>
        <b>Responsables de programmes :</b> suivi des objectifs, justification des écarts et plans d’action.<br>
        <b>Secrétariat Général :</b> arbitrage des situations critiques et suivi des engagements.<br>
        <b>Directions métiers/support :</b> alimentation des données et exécution des actions correctives.
        </div>
        """, unsafe_allow_html=True)

    st.markdown('<div class="section-title">Limites de la version démonstrateur</div>', unsafe_allow_html=True)
    st.markdown("""
    <div style="background:white;border:1px solid #E5E7EB;border-radius:12px;padding:18px;line-height:1.8;">
    Cette version constitue un démonstrateur fonctionnel de cadrage. Les données utilisées sont démonstratives.
    Une version opérationnelle nécessiterait une gouvernance des accès, une connexion aux systèmes sources,
    un circuit de validation des données, une politique de sécurité et un hébergement conforme aux exigences institutionnelles.
    </div>
    """, unsafe_allow_html=True)
    ...
    with right_assistant:
        ...
        <div class="assistant-panel">
          <div class="assistant-header">
            <div class="assistant-title">Assistant décisionnel SIICG</div>
            <div class="assistant-badge">AIDE À LA DÉCISION</div>
          </div>
...
"""
        ...
        # ============================================================
        st.markdown('<div class="section-title">Génération du rapport exécutif PDF</div>', unsafe_allow_html=True)

        st.markdown("""
        <div class="method-box">
        Ce module génère automatiquement un rapport exécutif structuré, exploitable dans une logique de revue de performance :
        synthèse, KPI prioritaires, décisions proposées, recommandations et lecture gouvernance.
        </div>
        """, unsafe_allow_html=True)
        ...
        def generate_executive_pdf_report():
            ...
            pdf_path = "SIICG_Executive_Performance_Report.pdf"
            ...
            elements.append(Paragraph("SIICG — Executive Performance Review Report", styles["ConsultingTitle"]))
            elements.append(Paragraph("Plateforme de pilotage de la performance publique · Version de cadrage fonctionnel", styles["BodyConsulting"]))
            elements.append(Paragraph(f"Date de génération : {datetime.now().strftime('%d/%m/%Y')}", styles["BodyConsulting"]))
            ...
            summary_data = [
                ["Indicateur", "Valeur"],
                ["Performance moyenne", f"{avg_perf:.1f}%"],
                ["KPI critiques", str(nb_rouge)],
                ["KPI sous vigilance", str(nb_orange)],
                ["KPI conformes", str(nb_vert)],
                ["Niveau global RPM", str(rpm_priority)],
            ]
            ...
            # Strategic narrative
            elements.append(Paragraph("2. Lecture stratégique", styles["SectionTitle"]))
            narrative = f"""
            Le portefeuille de performance présente un taux moyen de réalisation de <b>{avg_perf:.1f}%</b>.
            La lecture des indicateurs fait ressortir <b>{nb_rouge}</b> KPI critique(s) et <b>{nb_orange}</b> KPI sous vigilance.
            Ces éléments doivent être examinés dans le cadre de la prochaine Revue de Performance Mensuelle afin de
            documenter les écarts, formaliser les plans d’action et arbitrer les priorités de redressement.
            """
            elements.append(Paragraph(narrative, styles["BodyConsulting"]))
            elements.append(Spacer(1, 12))

            # KPI priority table
            elements.append(Paragraph("3. KPI prioritaires à inscrire à l’ordre du jour", styles["SectionTitle"]))

            kpi_table_data = [["Programme", "KPI", "Taux", "Statut", "Niveau d’escalade", "Responsable"]]
            for _, row in worst_df.iterrows():
                kpi_table_data.append([
                    str(row["programme"]),
                    str(row["code_kpi"]),
                    f"{row['taux_realisation_pct']:.1f}%",
                    str(row["statut"]),
                    str(row["niveau_alerte"]),
                    str(row["responsable"]),
                ])

            kpi_table = Table(kpi_table_data, colWidths=[2.6*cm, 2.2*cm, 1.6*cm, 1.8*cm, 4.2*cm, 3.4*cm])
            ...
            # Recommendations
            elements.append(Paragraph("4. Recommandations & décisions proposées", styles["SectionTitle"]))
            recommendations = """
            • Prioriser les KPI rouges et les inscrire à l’ordre du jour de la prochaine RPM.<br/>
            • Demander aux responsables concernés un plan d’action correctif documenté.<br/>
            • Fixer une échéance de suivi et un niveau d’escalade pour chaque KPI sous tension.<br/>
            • Consolider les analyses DOCG avant arbitrage du Secrétariat Général.<br/>
            • Renforcer la traçabilité des décisions et leur suivi dans les rapports mensuels.
            """
            elements.append(Paragraph(recommendations, styles["BodyConsulting"]))
            elements.append(Spacer(1, 14))

            # Governance view
            elements.append(Paragraph("5. Lecture gouvernance SIICG", styles["SectionTitle"]))
            governance = """
            Le SIICG vise à dépasser une logique de reporting statique pour structurer un dispositif intégré de pilotage.
            La valeur ajoutée réside dans la capacité à relier les indicateurs, les alertes, les niveaux d’escalade,
            les plans d’action et le référentiel documentaire dans une même architecture décisionnelle.
            """
            elements.append(Paragraph(governance, styles["BodyConsulting"]))
            elements.append(Spacer(1, 18))

            footer = """
            <font size="8" color="#6B7280">
            SIICG — Démonstrateur fonctionnel · Usage interne · Données démonstratives · 2026
            </font>
            """
            elements.append(Paragraph(footer, styles["BodyConsulting"]))

            doc.build(elements)
            return pdf_path

        if st.button("📄 Générer le rapport exécutif PDF", width="stretch"):
            pdf_file = generate_executive_pdf_report()

            if pdf_file is None:
                st.error("Le module PDF nécessite la librairie reportlab. Ajoutez 'reportlab' dans requirements.txt puis redémarrez l’application.")
            else:
                with open(pdf_file, "rb") as f:
                    st.download_button(
                        "⬇ Télécharger le rapport exécutif PDF",
                        data=f.read(),
                        file_name="SIICG_Executive_Performance_Report.pdf",
            ...
            for col, doc in zip(cols, filtered_docs[i:i+3]):
                with col:
                    ...
                    for f in possible_files:
                        if doc["title"].lower().split()[0] in f.name.lower()                            or doc["category"].lower() in f.name.lower()                            or doc["file"].replace(".pdf","").lower()[:12] in f.name.lower():
                            matched_file = f
                            break
                    ...
                    if matched_file:
                        with open(matched_file, "rb") as f:
                            st.download_button(
                                ...
                                mime="application/pdf",
                                width="stretch",
                                key=f"download_{i}_{doc['title']}_{matched_file.name}"
                            )
                    else:
                        st.info("Document disponible dans le référentiel uploadé.")
